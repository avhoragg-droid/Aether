(function () {
    ...
    function initFilters() {
        ...
        if (quickSearchInput && nftGrid) {
            ...
            // Скрываем нижнюю панель при фокусе
            quickSearchInput.addEventListener('focus', () => {
                if (bottomNav) bottomNav.style.display = 'none';
            });
            
            // Показываем нижнюю панель при потере фокуса
            quickSearchInput.addEventListener('blur', () => {
                if (bottomNav) bottomNav.style.display = '';
            });
        }
        
        // Клик вне поиска - скрыть клавиатуру
        ...
        if (filterExpandBtn && priceFilterPanel) {
            filterExpandBtn.addEventListener('click', () => {
                ...
                // При первом открытии обновляем счётчик
        ...
        if (priceFilterSlider && nftGrid) {
            const updatePriceFilter = () => {
                ...
                // Если 0 или максимум - показываем все
    ...
    // Создание карточки без индивидуальных обработчиков (делегирование на контейнере)
    function createNFTCard(nftLink, price = 0, index = 0) {
        ...
        // Сохраняем данные в dataset для делегирования
        ...
        card.innerHTML = `
            <div class="nft-card-image" id="nft-image-${index}">
                <img src="${previewUrl}" alt="${name}" class="nft-preview-img" decoding="async">
            </div>
    ...
    // Делегированный обработчик для всех карточек NFT
    ...
    function initNFTGridDelegation() {
        ...
        container.addEventListener('click', (e) => {
            ...
            if (cartBtn) {
                ...
                // Клик на кнопку цены — открываем drawer
                e.stopPropagation();
                if (tg.HapticFeedback) {
                    tg.HapticFeedback.impactOccurred('medium');
                }
    ...
    function renderNFTCards(containerId, nftLinks, reset = true) {
        ...
        // Показать кнопку "Показать ещё" если есть ещё карточки
    ...
    function initNFTCards() {
        ...
        if (links.length > 0) {
            ...
            // Preload первых 50 изображений (первая страница)
            if (window.preloadGiftImages) {
                window.preloadGiftImages(shuffledLinks.slice(0, 50), 8);
            }
